...
export function MousePathCanvas({
  ...
  useEffect(() => {
    ...
    const drawPoint = (point: TrailPoint) => {
      ...
      gradient.addColorStop(0.5, `rgba(${baseR},${baseG},${baseB},${point.alpha * 0.6})`);
      gradient.addColorStop(0.8, `rgba(${baseR},${baseG},${baseB},${point.alpha * 0.2})`);
    ...
    const render = () => {
      ...
      if (hasMouseMoved) {
        ...
        if (trailPointsRef.current.length > maxTrailLength) {
          trailPointsRef.current = trailPointsRef.current.slice(0, maxTrailLength);
        }
